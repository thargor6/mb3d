...
type
  ...
  {$IFNDEF UseDelphi}
    ...
    TResourceStream = class(TStream)
      constructor Create(Instance: HInst; const ResName: AnsiString; ResType:PByte);
    private
  ...
  private
    ...
    function RGB2Quad(RGB: pRGBPixel): TRGBQuad;
  protected
...
{$IFNDEF UseDelphi}
  ...
  begin
    FindFile := FindFirstFile(pByte(Filename), FindData);
...
begin
  ...
  if (fMemory = nil) and (Size > 0) then
    ...
    if Size > 0 then  {Only realloc if the new size is greater than 0}
      ReallocMem(fMemory, Size * SIZEOF(Pointer))   //not a good memory usage?
    else
...
function TStream.GetSize: Longint;
  ...
  begin
    ...
    {Create handle}
    fHandle := CreateFile(pByte(Filename), OpenMode, FILE_SHARE_READ or
  ...
  begin
    ...
    ResID := FindResource(hInstance, pByte(ResName), RT_RCDATA);
...
begin
  ...
  begin
    Output := nil;
    if DecompressZLIB(pByte(Longint(Data) + Length(fKeyword) + 2),
...
{  PRGBTriple = ^TRGBTriple;   from: windows.pas
